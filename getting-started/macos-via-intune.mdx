---
title: "Deploy the Origin macOS Agent via Microsoft Intune"
sidebarTitle: "macOS via Intune"
description: "Push the Origin macOS agent silently to managed endpoints using Microsoft Intune — one PPPC profile, a JWT staging script, and a macOS PKG app deployment."
---

A step-by-step walkthrough for silent deployment of the Origin macOS agent — configuration profile, JWT staging script, and end-to-end Microsoft Intune workflow.
...
- **The configuration profile and install scripts** bundled with this guide (see files inventory in Step 03).
...
<Note>
  **Where to find it**
  ...
  Both the `.pkg` installer and the provisioning JWT live in the same place in the Origin console: **Settings → Installers**. Grab both before moving into Intune.

  Download the configuration profile from [MDM Overview](/getting-started/mdm-overview): [**origin-privacy.mobileconfig.txt**](/files/origin-privacy.mobileconfig.txt) — rename it to `origin-privacy.mobileconfig` after downloading. The JWT staging script is provided in full in Step 05 below.
</Note>
...
The configuration profile is downloadable from the docs site. The JWT staging script is reproduced in full in Step 05 — copy it straight into Intune.
...
3. **The Origin .pkg (via macOS app deployment)** (`Origin.pkg`) — The installer itself. Deployed as a macOS app (PKG) with no pre-install script. The `.pkg` postinstall reads the JWT staged in step 2 and registers silently. See Step 05 for the full app setup.

## Step 05 — Intune Walkthrough

The following sequence assumes you have the `origin-privacy.mobileconfig` profile plus the `.pkg` and the JWT staging shell script staged locally. The whole flow happens inside the Microsoft Intune admin center at `intune.microsoft.com`.

### 01. Upload the configuration profile

The `origin-privacy.mobileconfig` file must be uploaded as a **Custom** configuration profile. Do not use Intune's built-in PPPC template — it reconstructs the XML and breaks the code requirement that makes PPPC allow-listing work.
...
<Warning>
  **Verify before proceeding**
  ...
  Before moving to the package, confirm the Origin privacy profile is installed using either method:
  ...
  **On the Mac:** System Settings → General → Device Management, or run:
  ...
  This should return the Origin privacy profile. Do not proceed until it is confirmed installed on the target device.
</Warning>
...
<Note>
  **Intune Management Agent prerequisite**
  ...
  The macOS app (PKG) deployment type requires the Microsoft Intune Management Agent (version 2309.007 or greater) on the endpoint. The IME installs automatically on the first script- or PKG-type assignment, so the JWT staging script in Substep 02 should already have triggered it. If that script never reported **Succeeded**, the IME may not be healthy on the device — diagnose before continuing (see Step 07).
</Note>
...
<Warning>
  **Required ordering before assigning the PKG app**
  ...
  1. The privacy configuration profile (Substep 01) should be assigned to your full pilot/rollout group from the start.
  2. The JWT staging script (Substep 02) should also be assigned broadly — it's idempotent, so over-assigning is safe.
  3. The macOS PKG app (this substep) should be assigned to a narrower sub-group that you populate only after a device shows **Succeeded** on both the privacy profile and the JWT staging script. For pilot validation, populate the sub-group manually one device at a time. For broader rollout, automate population via a dynamic group whose membership rule keys off a custom attribute the JWT script writes on success.
</Warning>

### 05. Trigger the install

Once a device is in scope for the macOS PKG app (with the upstream privacy profile and JWT staging script already **Succeeded**), the install runs automatically on the next device check-in. To force it for testing:
...
**End-to-end flow:** the privacy profile applies → the JWT staging script writes `/var/tmp/origin-provisioning-jwt` (installing the Intune Management Agent on its first run) → Intune Management Agent installs the `.pkg` → the postinstall reads the staged JWT, registers silently, and deletes the file. No user interaction is required at any step.
...
<Note>
  **Intune admin center verification**
  ...
  Devices → All devices → select the target Mac → **Managed Apps**. The Origin app should show install status **Installed**. Under **Device configuration**, the Origin privacy profile should show **Succeeded**.
</Note>
...
A failure on the very first script assignment usually indicates the Intune Management Agent itself isn't healthy on this device yet. Check `ls /Library/Intune/` — if empty or missing the agent app bundle, force a sync and wait 5–10 minutes for the agent to install, then let the idempotent script retry.
