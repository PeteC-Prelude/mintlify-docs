...
- A **Claude Enterprise plan** with the Compliance API available. The Compliance API is generally available on Claude Enterprise, except for Public Sector organizations.
...
<Steps>
  <Step title="Enable the Compliance API in Claude">
  ...
  <Step title="Create a Compliance Access Key">
    ...
    2. Click **Create key** and give it a name — for example, `Origin`.
    3. Select these three scopes:
       - `read:compliance_activities`
       - `read:compliance_user_data`
       - `read:compliance_org_data`
    4. Click **Create** and copy the key immediately. It begins with `sk-ant-api01-`, and Claude shows it only once — store it somewhere safe before closing the dialog.

    Origin uses all three scopes: **activities** to find what changed, **user data** to read chats and their files, and **org data** to attribute activity to the right people. Leave `delete:compliance_user_data` unselected — Origin does not use it.

    <Warning>
      Scopes are fixed once a key is created. If a required scope is missing, delete the key and create a new one with all three read scopes selected.
    </Warning>
  </Step>

  <Step title="Save the key in Origin">
    ...
    Origin stores the key encrypted and does not show it again. Once it is saved, the section reads **Configured** and reports the current sync status.
  </Step>
</Steps>

## What to expect after connecting

Origin begins pulling data within a few minutes of the key being saved. New Claude activity then flows continuously into your traces, analytics, and inventory alongside the endpoint record. No further setup is required.
...
| Sync reports a 403 error | The Compliance API is not enabled, or the key is missing a scope | Confirm the Compliance API is enabled in claude.ai and the key holds all three read scopes. If a scope is missing, delete the key and create a new one. |
| No Claude activity appears | The key is valid but there is no recent Claude usage, or none within the backfill window | Use Claude, then wait for the next sync cycle. |
| Sync stops after previously working | The key was deleted or revoked in claude.ai | Create a new key in claude.ai and click **Replace key** in Origin. |

## What Origin accesses

Origin's access is intentionally narrow and read-only:

- **Read access** to your Claude organization through the three scopes listed above: chats, messages, files, projects, artifacts, and activity feed events.
